...
import { getSelfPlacementStatus, searchSchools, getProgrammes, submitSelfPlacement } from '../services/api'
  ...
  return (
    <nav className={s.authNav}>
      <div className={s.navInner}>
        <button className={s.navLink} onClick={() => navigate('/hub')}>Dashboard</button>
        <button className={s.navLink} onClick={() => navigate('/hub/placement')}>Placement Information</button>
        <button className={`${s.navLink} ${s.navLinkActive}`}>Self Placement</button>
      </div>
    </nav>
  )
}
...
function ArrowRightIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" aria-hidden="true"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
}
...
export default function SelfPlacement() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      await submitSelfPlacement({ school_id: selectedSchool.id, programme_code: selectedProg, residency })
    ...
    <div className={s.page}>
      ...
      <main className={s.main}>
        <div className="card">
          ...
          <p className={s.pageSub}>For candidates who are not placed after auto placement</p>
          ...
          {!loading && status === 'placed' && (
            <div className={s.stateCenter}>
              ...
              <button className="btn-gold" onClick={() => navigate('/hub/placement')}>
          ...
          {!loading && status === 'ineligible' && (
            <div className="alert alert-warn">
              ...
              Self placement is not currently available. Please contact GES for assistance.
            </div>
          )}
          ...
          {!loading && status === 'eligible' && !submitSuccess && (
            <div className={s.form}>
              ...
              <div className={s.field}>
                <label className={s.label} htmlFor="programme">Programme</label>
                <select
                  id="programme"
                  value={selectedProg}
                  onChange={(e) => { setSelectedProg(e.target.value); setSelectedSchool(null); setQuery('') }}
              ...
              {/* School search */}
              <div className={s.field}>
                <label className={s.label} htmlFor="school-search">Search School</label>
                ...
                {schools.length > 0 && !selectedSchool && (
                  <div className={s.dropdown}>
                    {schools.map((sc) => (
                      <button key={sc.id} type="button" className={s.dropdownItem} onClick={() => selectSchool(sc)}>
                        <strong>{sc.name}</strong>
                        <span className={s.schoolMeta}>{sc.district}, {sc.region}</span>
                      </button>
                    ))}
                  </div>
                )}
              ...
              {selectedSchool && (
                <div className={s.confirmBanner}>
                  ...
                  <span>{selectedSchool.district} · {selectedSchool.region}</span>
                </div>
              )}
              ...
              >
                {submitting && <span className="spinner" style={{width:14,height:14,borderTopColor:'#fff',borderColor:'rgba(255,255,255,0.3)'}} />}
          ...
          {submitSuccess && (
            <div className={s.stateCenter}>
              <svg className={s.successIcon} width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden="true"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
              <p>Placement Submitted!</p>
              <p className="note">Your self-placement request has been received. You will be notified once it is processed.</p>
